...
struct HashTableInt {
    ...
    void prepare(hipStream_t stream) {
       ...
       hipLaunchKernelGGL(( prepare_key_value_pairs_kernel), dim3(blocks), dim3(tpb), 0, stream, table_kvs.data(), capacity);
    }
    ...
    void insert(const int* d_coords_in, int n_keys, hipStream_t stream) {
       ...
       hipLaunchKernelGGL(( insert_kernel), dim3(blocks), dim3(tpb), 0, stream, table_kvs.data(), d_coords, n_keys, num_dims, capacity);
    }
    ...
    void search(const int* d_queries, int n_queries, int* d_out_indices, hipStream_t stream) const {
       ...
       hipLaunchKernelGGL(( search_kernel), dim3(blocks), dim3(tpb), 0, stream, table_kvs.data(), d_coords, d_queries, n_queries, capacity, num_dims, d_out_indices);
    }
};
